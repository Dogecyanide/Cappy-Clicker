...
check(PRODUCERS.length === 20, `Expected 20 producers, found ${PRODUCERS.length}.`);
check(BUILDING_UPGRADES.length === 140, `Expected 140 producer upgrades, found ${BUILDING_UPGRADES.length}.`);
check(ACHIEVEMENTS.length === 250, `Expected 250 achievements, found ${ACHIEVEMENTS.length}.`);
check(POWER_MOONS.length === 16, `Expected 16 Power Moons, found ${POWER_MOONS.length}.`);
...
for (const upgrade of BUILDING_UPGRADES) check(Boolean(PRODUCER_BY_ID[upgrade.producerId]), `Upgrade ${upgrade.id} references missing producer ${upgrade.producerId}.`);
for (const moon of POWER_MOONS) {
  if (moon.effect.type === 'producer-group') for (const id of moon.effect.producerIds) check(Boolean(PRODUCER_BY_ID[id]), `Moon ${moon.id} references missing producer ${id}.`);
}
